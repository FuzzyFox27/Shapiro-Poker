    ...
    class AI : Player
    {
        ...
        */
        ...
        bool[] EvaluateTable(Table table)
        {
            ...
            bool[] RiverCombination = new bool[] {false,false,false,false,false,false,false };
            ...
            switch (River.Count)
            {
                case 3:
                    ...
                    if (eval.CalculateGrade(River.ToArray(), Evaluate.Grades.ThreeOfAKind)) { RiverCombination.SetValue(true,3); }
                    if (eval.CalculateGrade(River.ToArray(), Evaluate.Grades.Pair)) { RiverCombination.SetValue(true, 0); }
                    break;
                case 4:
                    //4oak, 3oak,2p,p
                    if(eval.CalculateGrade(River.ToArray(), Evaluate.Grades.FourOfAKind)) { RiverCombination.SetValue(true, 3); }
                    if(eval.CalculateGrade(River.ToArray(), Evaluate.Grades.ThreeOfAKind)) { RiverCombination.SetValue(true, 2); }
                    if(eval.CalculateGrade(River.ToArray(), Evaluate.Grades.TwoPairs)) { RiverCombination.SetValue(true, 1); }
                    if (eval.CalculateGrade(River.ToArray(), Evaluate.Grades.Pair)) { RiverCombination.SetValue(true, 0); }
                    break;
                case 5:
                    //sf,f,s,4oak,3oak,2p,p
                    if(eval.CalculateGrade(River.ToArray(), Evaluate.Grades.StraightFlush)) { RiverCombination.SetValue(true, 6); }
                    if(eval.CalculateGrade(River.ToArray(), Evaluate.Grades.Flush)) { RiverCombination.SetValue(true, 5); }
                    if (eval.CalculateGrade(River.ToArray(), Evaluate.Grades.Straight)) { RiverCombination.SetValue(true, 4); }
                    if (eval.CalculateGrade(River.ToArray(), Evaluate.Grades.FourOfAKind)) { RiverCombination.SetValue(true, 3); }
                    if (eval.CalculateGrade(River.ToArray(), Evaluate.Grades.ThreeOfAKind)) { RiverCombination.SetValue(true, 2); }
                    if (eval.CalculateGrade(River.ToArray(), Evaluate.Grades.TwoPairs)) { RiverCombination.SetValue(true, 1); }
                    if (eval.CalculateGrade(River.ToArray(), Evaluate.Grades.Pair)) { RiverCombination.SetValue(true, 0); }
